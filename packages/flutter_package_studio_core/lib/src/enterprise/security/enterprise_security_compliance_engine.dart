...
class EnterpriseSecurityComplianceEngine {
  ...
  Future<SecurityComplianceAssessmentResult> assessCompliance({
    ...
    _logger.info('Starting security compliance audit for profile: ${activePolicy.profile.displayName}');

    // ─────────────────────────────────────────────────────────────────────────
    // Control 1: Secret Detection & Redaction Requirements
    // ─────────────────────────────────────────────────────────────────────────
    var secretsPassed = true;
    final detectedSecretsCount = (operationalEvidence?['detected_secrets_count'] as int?) ?? 0;
    if (activePolicy.requireZeroSecrets && detectedSecretsCount > 0) {
      findings.add(ComplianceControlFinding(
        ...
        description: 'Discovered $detectedSecretsCount unredacted secret(s) in repository assets or source files.',
        remediation: 'Redact all tokens, private keys, and credentials using SecretRedactor.',
        isBlocking: true,
      ));
      secretsPassed = false;
      ...
      findings.add(const ComplianceControlFinding(
        ...
        description: 'Zero plaintext secrets detected across inspected codebase.',
      ));
    ...
    // ─────────────────────────────────────────────────────────────────────────
    var sensitiveFilesPassed = true;
    final filter = SensitiveFileFilter.fromProjectRoot(_projectRoot);
    final envFile = File(p.join(_projectRoot, '.env'));
    final keyFile = File(p.join(_projectRoot, 'android', 'key.properties'));

    if (activePolicy.enforceSensitiveFileExclusion && (envFile.existsSync() || keyFile.existsSync())) {
      findings.add(ComplianceControlFinding(
        controlId: 'SEC_CTL_002_SENSITIVE_FILE_BOUNDARY',
        controlTitle: 'Sensitive File Exclusion Boundary',
        category: 'File Exposure',
        status: ComplianceStatus.failed,
        description: 'Found sensitive configuration files (.env or key.properties) inside project workspace root.',
        remediation: 'Exclude sensitive files from version control and ensure they are added to .gitignore.',
        affectedResource: envFile.existsSync() ? '.env' : 'key.properties',
        isBlocking: true,
      ));
      sensitiveFilesPassed = false;
    ...
    // ─────────────────────────────────────────────────────────────────────────
    final credentialsInSource = (operationalEvidence?['credentials_in_source'] as bool?) ?? false;
    if (activePolicy.requireCredentialIsolation && credentialsInSource) {
      findings.add(const ComplianceControlFinding(
        ...
        status: ComplianceStatus.failed,
        description: 'Detected raw credentials stored in code or configuration files.',
        remediation: 'Use enterprise credential provider abstraction or OS credential vault.',
        isBlocking: true,
      ));
    ...
    // ─────────────────────────────────────────────────────────────────────────
    final hasEncryptedArtifacts = (operationalEvidence?['encrypted_artifacts_verified'] as bool?) ?? true;
    if (activePolicy.requireEncryptedArtifacts && !hasEncryptedArtifacts) {
      findings.add(ComplianceControlFinding(
        ...
        category: 'Encryption',
        status: ComplianceStatus.failed,
        description: 'Profile "${activePolicy.profile.displayName}" mandates cryptographic encryption and signature verification on all release archives.',
        remediation: 'Enable artifact encryption and package signing before release distribution.',
        isBlocking: true,
      ));
      ...
      findings.add(const ComplianceControlFinding(
        ...
        category: 'Encryption',
        status: ComplianceStatus.passed,
        description: 'Encryption controls verified or not required for standard profile.',
      ));
    ...
    // ─────────────────────────────────────────────────────────────────────────
    final criticalVulnerabilities = (operationalEvidence?['critical_vulnerabilities'] as int?) ?? 0;
    final highVulnerabilities = (operationalEvidence?['high_vulnerabilities'] as int?) ?? 0;
    final mediumVulnerabilities = (operationalEvidence?['medium_vulnerabilities'] as int?) ?? 0;
    ...
    if (criticalVulnerabilities > activePolicy.maxAllowedCriticalFindings) {
      findings.add(ComplianceControlFinding(
        ...
        status: ComplianceStatus.failed,
        description: 'Found $criticalVulnerabilities critical vulnerabilities (max permitted: ${activePolicy.maxAllowedCriticalFindings}).',
      ...
      findings.add(ComplianceControlFinding(
        ...
        category: 'Vulnerability Management',
        status: ComplianceStatus.failed,
        description: 'Found $highVulnerabilities high vulnerabilities (max permitted: ${activePolicy.maxAllowedHighFindings}).',
      ...
      findings.add(const ComplianceControlFinding(
        ...
        category: 'Vulnerability Management',
        status: ComplianceStatus.passed,
        description: 'Vulnerability counts conform strictly to profile threshold limits.',
      ));
    ...
    // ─────────────────────────────────────────────────────────────────────────
    final hasSecurityReview = (operationalEvidence?['formal_security_review_completed'] as bool?) ?? true;
    if (activePolicy.requireFormalSecurityReview && !hasSecurityReview) {
      findings.add(ComplianceControlFinding(
        ...
        status: ComplianceStatus.failed,
        description: 'Formal security auditor sign-off is mandatory for profile "${activePolicy.profile.displayName}".',
        remediation: 'Request formal review from designated Security Auditor role.',
        isBlocking: true,
      ));
    ...
    final criticalCount = findings.where((f) => f.status == ComplianceStatus.failed && f.isBlocking).length;
    final highCount = findings.where((f) => f.status == ComplianceStatus.failed && !f.isBlocking).length;
    final mediumCount = findings.where((f) => f.status == ComplianceStatus.warning).length;
