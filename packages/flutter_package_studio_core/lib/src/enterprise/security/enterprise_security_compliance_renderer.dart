...
class EnterpriseSecurityComplianceRenderer {
  ...
  String renderMarkdown(SecurityComplianceAssessmentResult result) {
    ...
    buffer.writeln('**Overall Compliance Status**: $statusIcon **${result.isCompliant ? "COMPLIANT" : (result.isBlocked ? "BLOCKED (POLICY VIOLATION)" : "NON-COMPLIANT")}**  ');
    buffer.writeln('**Total Controls Evaluated**: `${result.totalControlsEvaluated}`  ');
    ...
    buffer.writeln('## Evaluated Security Compliance Controls (${result.controlFindings.length})');
    for (final finding in result.controlFindings) {
      ...
      buffer.writeln('### $icon [${finding.controlId}] ${finding.controlTitle} (`${finding.category}`)');
      buffer.writeln('- **Status**: `${finding.status.label}`');
      buffer.writeln('- **Description**: ${finding.description}');
      if (finding.affectedResource != null) {
        buffer.writeln('- **Affected Resource**: `${finding.affectedResource}`');
      }
