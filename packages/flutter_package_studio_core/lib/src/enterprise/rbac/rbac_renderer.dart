...
class RbacRenderer {
  ...
  String renderMarkdown(AuthorizationResult result) {
    ...
    buffer.writeln();
    buffer.writeln('**Identity**: `${result.identity.displayName}` (`${result.identity.id}`)  ');
    buffer.writeln('**Operation**: `${result.operation.name}` (`${result.operation.permissionKey}`)  ');
    buffer.writeln('**Decision**: ${result.decision == AuthorizationDecision.allow ? "✅ ALLOW" : (result.decision == AuthorizationDecision.requiresApproval ? "⚠️ REQUIRES APPROVAL (✓*)" : "❌ DENY")}  ');
    ...
    buffer.writeln('Requires Approval: ${result.requiresAdditionalApproval ? "YES" : "NO"}');
    buffer.writeln('Reason: ${result.reason}');
    buffer.writeln('Authorized Roles: ${result.authorizedRoles.map((r) => r.displayName).join(", ")}');
    buffer.writeln('```');
    buffer.writeln();
