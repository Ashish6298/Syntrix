...
class StudioV2Renderer {
  ...
  static String renderAsciiLayout(StudioV2State state) {
    ...
    buffer.writeln('┌────────────────────────────────────────────────────────────────────────┐');
    buffer.writeln('│  Syntrix Studio v2 Shell  │  Section: ${state.currentSection.label.padRight(28)} │');
    buffer.writeln('├────────────────────────────────────────────────────────────────────────┤');
    buffer.writeln('│ [Toolbar] Reset (Ctrl+R) │ Pause (Space) [${state.isLivePreviewPaused ? "PAUSED " : "PLAYING"}] │ Diag: ${state.isDiagnosticsOverlayVisible ? "ON " : "OFF"} │');
    buffer.writeln('├──────────────────────────────┬─────────────────────────────────────────┤');
    buffer.writeln('│  Navigation Rail             │  Live Viewport Preview                  │');
    buffer.writeln('│  -------------------         │  ----------------------                 │');
    for (final sec in StudioNavigationSection.values.take(6)) {
      final marker = sec == state.currentSection ? '►' : ' ';
      buffer.writeln('│ $marker ${sec.label.padRight(26)} │ Target: ${config.targetLoaderId.padRight(31)} │');
    }
    buffer.writeln('├──────────────────────────────┼─────────────────────────────────────────┤');
    buffer.writeln('│  Configuration Inspector     │  Console & Telemetry Output             │');
    buffer.writeln('│  -----------------------     │  --------------------------             │');
    buffer.writeln('│  Speed:     ${config.animationSpeed.toStringAsFixed(1).padRight(16)} │  Theme:     ${config.selectedThemeId.padRight(27)} │');
    buffer.writeln('│  Intensity: ${config.intensity.toStringAsFixed(1).padRight(16)} │  Particles: ${config.particleCount.toString().padRight(27)} │');
    buffer.writeln('│  Scale:     ${config.scale.toStringAsFixed(1).padRight(16)} │  Interactive: ${config.isInteractive.toString().padRight(25)} │');
    buffer.writeln('└──────────────────────────────┴─────────────────────────────────────────┘');
  ...
  static String renderMarkdown(StudioV2State state) {
    ...
    buffer.writeln('**Current Navigation Section:** `${state.currentSection.label}`  ');
    buffer.writeln('**Live Preview Status:** `${state.isLivePreviewPaused ? "PAUSED" : "ACTIVE"}`  ');
    buffer.writeln('**Diagnostics Overlay:** `${state.isDiagnosticsOverlayVisible ? "ENABLED" : "DISABLED"}`  ');
    buffer.writeln('**Last Modified:** ${state.lastModifiedAt.toIso8601String()}');
    buffer.writeln();
    ...
    for (final p in state.registeredPanels) {
      buffer.writeln('| `${p.panelId}` | ${p.title} | `${p.area.id}` | ${p.isCollapsible ? "Yes" : "No"} |');
    }
    buffer.writeln();
