...
class StudioPersistenceRenderer {
  ...
  static String renderJson(StudioPersistentProjectState state,
      {bool pretty = true}) {
    final encoder =
  ...
  static String renderAsciiProjectCard(StudioPersistentProjectState state) {
    ...
    buffer.writeln(
        '│ Selected Loader:  ${state.selectedLoaderId.padRight(36)} │');
    buffer
        .writeln('│ Selected Theme:   ${state.selectedThemeId.padRight(36)} │');
    buffer.writeln(
        '│ Scene:            ${state.activeScene.name.padRight(36)} │');
    buffer.writeln(
        '│ Presets Loaded:   ${state.presets.length.toString().padRight(36)} │');
    buffer.writeln(
        '│ Last Saved:       ${state.lastSavedAt.toIso8601String().padRight(36)} │');
    buffer
        .writeln('├────────────────────────────────────────────────────────┤');
    buffer.writeln(
        '│ Config: Speed ${cfg.animationSpeed}x | Particles ${cfg.particleCount} | Shaders ${cfg.shadersEnabled ? "ON" : "OFF"}  │');
    buffer
        .writeln('│ Actions: [Save State]  [Load Project]  [Export JSON]   │');
    buffer
        .writeln('└────────────────────────────────────────────────────────┘');
  ...
  static String renderMarkdown(StudioPersistentProjectState state) {
    ...
    buffer.writeln(
        '**Active Scene:** `${state.activeScene.name}` (`${state.activeScene.layers.length}` layers)  ');
    ...
    buffer.writeln(
        '| **Interactive** | `${cfg.isInteractive ? "Enabled" : "Disabled"}` |');
    buffer.writeln();
