...
class StudioInspectorRenderer {
  ...
  static String renderAsciiInspector(StudioInspectorEngine engine) {
    ...
    for (final category in InspectorPropertyCategory.values) {
      ...
      for (final p in props) {
        ...
        if (p.controlType == InspectorControlType.slider && p.minValue != null && p.maxValue != null) {
          final sliderTrack = _renderSliderTrack(val as num, p.minValue!, p.maxValue!);
          buffer.writeln('│ ${p.label.padRight(12)} $sliderTrack  ${val.toStringAsFixed(1).padLeft(5)} │');
        } else if (p.controlType == InspectorControlType.toggleSwitch) {
          final toggle = (val == true) ? '[ON ]' : '[OFF]';
          buffer.writeln('│ ${p.label.padRight(12)} $toggle${"".padRight(28)} │');
        } else {
          buffer.writeln('│ ${p.label.padRight(12)} [${val.toString().padRight(20)}]${"".padRight(11)} │');
        }
      }
      buffer.writeln('│                                                        │');
    }
  ...
  static String _renderSliderTrack(num current, double min, double max, {int length = 20}) {
    if (max <= min) return '─────●──────────────';
  ...
  static String renderMarkdown(StudioInspectorEngine engine) {
    ...
    buffer.writeln('# Visual Configuration Inspector');
    buffer.writeln();
    buffer.writeln('Interactive property controls bound to active Studio v2 preview.');
    buffer.writeln();
    ...
    for (final category in InspectorPropertyCategory.values) {
      ...
      buffer.writeln();
      buffer.writeln('| Property | Control | Value | Bounds / Step | Description |');
      buffer.writeln('|---|:---:|:---:|:---:|---|');

      for (final p in props) {
        final val = engine.getPropertyValue(p.propertyKey);
        final bounds = (p.minValue != null && p.maxValue != null) ? '${p.minValue} - ${p.maxValue} (step ${p.step ?? 1})' : '—';
        buffer.writeln('| **${p.label}** (`${p.propertyKey}`) | `${p.controlType.id}` | `${val}` | $bounds | ${p.description} |');
      }
      buffer.writeln();
    }
