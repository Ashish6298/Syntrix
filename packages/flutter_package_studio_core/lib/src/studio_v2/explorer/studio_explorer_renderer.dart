...
class StudioExplorerRenderer {
  ...
  static String renderJson(List<LoaderMetadataEntry> loaders, {bool pretty = true}) {
  ...
  static String renderLoaderCardAscii(LoaderMetadataEntry loader, {bool isFavorite = false}) {
    ...
    buffer.writeln('├────────────────────────────────────────────────────────┤');
    buffer.writeln('│ Category:    ${loader.category.displayName.padRight(41)} │');
    buffer.writeln('│ Complexity:  ${loader.complexity.label.padRight(41)} │');
    buffer.writeln('│ Particles:   ${(loader.hasParticles ? "✓ Enabled" : "✗ None").padRight(41)} │');
    buffer.writeln('│ Physics:     ${(loader.hasPhysics ? "✓ Active" : "✗ None").padRight(41)} │');
    buffer.writeln('│ Shaders:     ${(loader.hasShaders ? "✓ GPU Accelerated" : "✗ Standard Canvas").padRight(41)} │');
    buffer.writeln('│ Interactive: ${(loader.isInteractive ? "✓ Gesture Enabled" : "✗ Static Loop").padRight(41)} │');
    buffer.writeln('├────────────────────────────────────────────────────────┤');
    buffer.writeln('│ Actions: [Preview]  [Configure]  [Generate Code] [Export] │');
    buffer.writeln('└────────────────────────────────────────────────────────┘');
  ...
  static String renderMarkdownCatalog(List<LoaderMetadataEntry> loaders, {Set<String> favorites = const {}}) {
    final buffer = StringBuffer();
    ...
    buffer.writeln('| Loader Name | Category | Complexity | Particles | Physics | Shaders | Interactive | Favorite |');
    buffer.writeln('|---|---|:---:|:---:|:---:|:---:|:---:|:---:|');
    for (final l in loaders) {
      final isFav = favorites.contains(l.id) ? '★' : '☆';
      buffer.writeln('| **${l.name}** (`${l.id}`) | ${l.category.displayName} | `${l.complexity.label}` | ${l.hasParticles ? "✓" : "—"} | ${l.hasPhysics ? "✓" : "—"} | ${l.hasShaders ? "✓" : "—"} | ${l.isInteractive ? "✓" : "—"} | $isFav |');
    }
    buffer.writeln();
    ...
    for (final l in loaders) {
      ...
      buffer.writeln('- **Supported Themes:** ${l.supportedThemes.map((t) => "`$t`").join(", ")}');
      buffer.writeln();
    }
  ...
  static String renderComparisonMarkdown(Map<String, dynamic> comparisonResult) {
    ...
    buffer.writeln('| **Complexity** | ${a["complexity"]} | ${b["complexity"]} |');
    buffer.writeln('| **Particles** | ${a["has_particles"] == true ? "✓" : "—"} | ${b["has_particles"] == true ? "✓" : "—"} |');
    buffer.writeln('| **Physics** | ${a["has_physics"] == true ? "✓" : "—"} | ${b["has_physics"] == true ? "✓" : "—"} |');
    buffer.writeln('| **Shaders** | ${a["has_shaders"] == true ? "✓" : "—"} | ${b["has_shaders"] == true ? "✓" : "—"} |');
    buffer.writeln('| **Interactive** | ${a["is_interactive"] == true ? "✓" : "—"} | ${b["is_interactive"] == true ? "✓" : "—"} |');
    buffer.writeln();
