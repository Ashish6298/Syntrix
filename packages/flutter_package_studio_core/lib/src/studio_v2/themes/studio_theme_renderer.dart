...
import 'package:flutter_package_studio_core/src/studio_v2/themes/studio_theme_engine.dart';
...
class StudioThemeRenderer {
  ...
  static String renderJson(List<StudioThemeDescriptor> themes, {bool pretty = true}) {
    final encoder = pretty ? const JsonEncoder.withIndent('  ') : const JsonEncoder();
    return encoder.convert(themes.map((t) => t.toJson()).toList());
  }

  /// Render ASCII visual theme card with color swatch preview.
  static String renderThemeCardAscii(StudioThemeDescriptor theme, {bool isActive = false}) {
    ...
    buffer.writeln('│ Palette Swatch:                                        │');
    final swatch = theme.palette.map((c) => '[${c.role.substring(0, 3).toUpperCase()}: ${c.hex}]').join(' ');
    buffer.writeln('│ ${swatch.padRight(54)} │');
    buffer.writeln('├────────────────────────────────────────────────────────┤');
    buffer.writeln('│ Loaders: ${theme.supportedLoaderIds.length} supported | Custom Theme: ${(theme.isCustom ? "Yes" : "Built-in").padRight(16)} │');
    buffer.writeln('│ Actions: [Apply]  [Customize]  [Duplicate]  [Gen Code] │');
    buffer.writeln('└────────────────────────────────────────────────────────┘');
  ...
  static String renderMarkdownCatalog(List<StudioThemeDescriptor> themes, StudioThemeEngine engine) {
    final buffer = StringBuffer();
    ...
    buffer.writeln('| Theme Name | ID | Background | Glow Color | Palette Roles | Custom |');
    buffer.writeln('|---|---|:---:|:---:|---|:---:|');
    for (final t in themes) {
      final roles = t.palette.map((c) => '`${c.role}`').join(', ');
      buffer.writeln('| **${t.name}** | `${t.id}` | `${t.backgroundColor}` | `${t.particleGlowColor}` | $roles | ${t.isCustom ? "Yes" : "Built-in"} |');
    }
    buffer.writeln();
  ...
  static String renderComparisonMarkdown(Map<String, dynamic> comp) {
    ...
    buffer.writeln('|---|---|---|');
    buffer.writeln('| **Background** | `${a["background_color"]}` | `${b["background_color"]}` |');
    buffer.writeln('| **Particle Glow** | `${a["particle_glow_color"]}` | `${b["particle_glow_color"]}` |');
    buffer.writeln('| **Particle Size** | `${a["default_particle_size"]}` | `${b["default_particle_size"]}` |');
    buffer.writeln('| **Particle Opacity** | `${a["default_particle_opacity"]}` | `${b["default_particle_opacity"]}` |');
    buffer.writeln();
