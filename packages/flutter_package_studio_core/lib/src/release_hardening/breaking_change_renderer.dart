...
class BreakingChangeRenderer {
  ...
  static String renderJson(BreakingChangeAuditReport report, {bool pretty = true}) {
    final encoder = pretty ? const JsonEncoder.withIndent('  ') : const JsonEncoder();
    return encoder.convert(report.toJson());
  }

  /// Render ASCII Breaking-Change Audit Dashboard.
  static String renderAsciiBreakingChangeDashboard(BreakingChangeAuditReport report) {
    final buffer = StringBuffer();

    buffer.writeln('┌────────────────────────────────────────────────────────────┐');
    buffer.writeln('│ PHASE 11.2 — BREAKING-CHANGE AUDIT MATRIX                  │');
    buffer.writeln('├────────────────────────────────────────────────────────────┤');
    ...
    buffer.writeln('├────────────────────────────────────────────────────────────┤');
    buffer.writeln('│ Dimensions Audited: ${report.totalDimensionsAudited.toString().padRight(2)} | Violations Found: ${report.totalViolationsFound.toString().padRight(2)}                    │');
    buffer.writeln('│ Backward Compatibility: ${(report.isBackwardCompatible ? "PASSED (100% Compatible)" : "FAILED (Violations Found)").padRight(34)} │');
    buffer.writeln('└────────────────────────────────────────────────────────────┘');
  ...
  static String renderMarkdown(BreakingChangeAuditReport report) {
    ...
    buffer.writeln();
    buffer.writeln('**Backward Compatibility Status:** `${report.isBackwardCompatible ? "PASSED (100% Compatible)" : "FAILED"}`  ');
    ...
    buffer.writeln();
    buffer.writeln('- **Total Dimensions Audited:** `${report.totalDimensionsAudited}`');
    buffer.writeln('- **Unintentional Breaking Changes Found:** `${report.totalViolationsFound}`');
    buffer.writeln('- **Sound Null Safety & API Contract Adherence:** `100% VERIFIED`');
    buffer.writeln();

    buffer.writeln('## Audited Dimensions Matrix');
    buffer.writeln();
    buffer.writeln('| Dimension | Status | Target Scope | Verification Details |');
    buffer.writeln('|---|:---:|---|---|');
    for (final item in report.auditItems) {
      buffer.writeln('| **${item.dimension.label}** | ${item.status.symbol} `${item.status.label}` | ${item.auditedTarget} | ${item.verificationDetails} |');
    }
    buffer.writeln();
