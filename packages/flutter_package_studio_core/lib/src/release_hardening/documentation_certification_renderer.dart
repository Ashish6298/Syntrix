...
class DocumentationCertificationRenderer {
  ...
  static String renderJson(DocumentationCertificationReport report, {bool pretty = true}) {
    final encoder = pretty ? const JsonEncoder.withIndent('  ') : const JsonEncoder();
    return encoder.convert(report.toJson());
  }

  /// Render ASCII Documentation Certification Matrix Dashboard.
  static String renderAsciiDocDashboard(DocumentationCertificationReport report) {
    final buffer = StringBuffer();

    buffer.writeln('┌────────────────────────────────────────────────────────────┐');
    buffer.writeln('│ PHASE 11.6 — DOCUMENTATION CERTIFICATION AUDIT             │');
    buffer.writeln('├────────────────────────────────────────────────────────────┤');
    buffer.writeln('│ Mandatory Files                                            │');
    buffer.writeln('├────────────────────────────────────────────────────────────┤');
    ...
    buffer.writeln('├────────────────────────────────────────────────────────────┤');
    buffer.writeln('│ Files: ${report.totalFilesAudited.toString().padRight(2)} | Topics: ${report.totalTopicsAudited.toString().padRight(2)} | Status: ${report.isDocCertified ? "CERTIFIED" : "FAILED"}                   │');
    buffer.writeln('│ Target Release: ${(report.isDocCertified ? "v${report.targetVersion} (DOCUMENTATION CERTIFIED)" : "DOCUMENTATION INCOMPLETE").padRight(42)} │');
    buffer.writeln('└────────────────────────────────────────────────────────────┘');
  ...
  static String renderMarkdown(DocumentationCertificationReport report) {
    ...
    buffer.writeln('# Milestone 11 — Phase 11.6: Documentation Certification Report');
    buffer.writeln();
    buffer.writeln('**Documentation Certification Status:** `${report.isDocCertified ? "CERTIFIED (100% Complete)" : "FAILED"}`  ');
    buffer.writeln('**Target Release Version:** `v${report.targetVersion}`  ');
    buffer.writeln('**Total Files Audited:** `${report.totalFilesAudited}`  ');
    buffer.writeln('**Total Topics Audited:** `${report.totalTopicsAudited}`  ');
    ...
    buffer.writeln('|---|:---:|---|');
    for (final item in report.fileItems) {
      buffer.writeln('| `${item.identifier}` | ${item.status.symbol} `${item.status.label}` | ${item.verificationDetails} |');
    }
    buffer.writeln();
