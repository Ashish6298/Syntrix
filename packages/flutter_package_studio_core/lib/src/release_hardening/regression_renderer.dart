...
class RegressionTestRenderer {
  ...
  static String renderAsciiRegressionDashboard(RegressionTestReport report) {
    ...
    buffer.writeln('┌────────────────────────────────────────────────────────────┐');
    buffer.writeln('│ PHASE 11.3 — FULL REGRESSION TEST MATRIX                   │');
    buffer.writeln('├────────────────────────────────────────────────────────────┤');
    ...
    buffer.writeln('├────────────────────────────────────────────────────────────┤');
    buffer.writeln('│ Total Tests Run: ${report.totalTestsRun.toString().padRight(4)} | Code Coverage: ${("${report.coveragePercentage.toStringAsFixed(1)}%").padRight(6)} | Status: ${report.isRegressionPassed ? "PASSED" : "FAILED"}   │');
    buffer.writeln('│ Target Release: ${(report.isRegressionPassed ? "v${report.targetVersion} (CERTIFIED)" : "BLOCKED").padRight(42)} │');
    buffer.writeln('└────────────────────────────────────────────────────────────┘');
  ...
  static String renderMarkdown(RegressionTestReport report) {
    ...
    buffer.writeln('# Milestone 11 — Phase 11.3: Full Regression Testing Report');
    buffer.writeln();
    buffer.writeln('**Regression Suite Status:** `${report.isRegressionPassed ? "PASSED (100% Green)" : "FAILED"}`  ');
    buffer.writeln('**Target Release Version:** `v${report.targetVersion}`  ');
    buffer.writeln('**Total Automated Tests Executed:** `${report.totalTestsRun}`  ');
    buffer.writeln('**Line Coverage:** `${report.coveragePercentage.toStringAsFixed(1)}%`  ');
    buffer.writeln('**Tested At:** ${report.testedAt.toIso8601String()}');
    buffer.writeln();

    buffer.writeln('## Regression Matrix Breakdown');
    buffer.writeln();
    buffer.writeln('| Category | Command | Status | Duration | Verification Details |');
    buffer.writeln('|---|---|:---:|:---:|---|');
    for (final item in report.testItems) {
      buffer.writeln('| **${item.category.label}** | `${item.command}` | ${item.status.symbol} `${item.status.label}` | ${item.durationMs}ms | ${item.verificationDetails} |');
    }
    buffer.writeln();
