...
import 'package:syntrix/src/commands/template_ai_command.dart';
...
// ─────────────────────────────────────────────────────────────────────────────
// template verify-release <template-id>
// ─────────────────────────────────────────────────────────────────────────────

/// Subcommand: `fps template verify-release <template-id>`
///
/// Previews or executes the release verification pipeline for a template.
class TemplateVerifyReleaseCommand extends FpsCommand {
  @override
  final String name = 'verify-release';

  @override
  final String description =
      'Preview or execute full release verification pipeline for a template.';

  TemplateVerifyReleaseCommand() {
    argParser.addOption(
      'version',
      abbr: 'v',
      help: 'Version constraint/target for template release verification.',
      defaultsTo: '1.0.0',
    );
    argParser.addOption(
      'profile',
      abbr: 'p',
      help: 'Verification policy profile (standard, strict, custom).',
      defaultsTo: 'standard',
    );
    argParser.addOption(
      'output',
      abbr: 'o',
      help: 'Target output directory when writing verification report to disk.',
      defaultsTo: 'doc/release',
    );
    argParser.addFlag(
      'write',
      negatable: false,
      help: 'Write generated release verification report directly to disk.',
    );
    argParser.addFlag(
      'json',
      negatable: false,
      help: 'Output release verification pipeline result as JSON.',
    );
  }
  ...
  Future<int> run() async {
    ...
    final options = ReleaseVerificationOptions(
      packageName: tmpl.manifest.name.toLowerCase().replaceAll(' ', '_'),
      version: targetVersion,
      profile: profile,
      outputDir: outputDir,
    ...
    if (writeDisk) {
      ...
      await baseDir.create(recursive: true);
      final file = File(
          '${baseDir.path}/release_verification_report.${jsonOutput ? 'json' : 'md'}');
      await file.writeAsString(
          jsonOutput ? jsonEncode(result.toJson()) : result.toMarkdown());

      if (!jsonOutput) {
        print(
            'Successfully wrote release verification report to "${file.path}".');
      }
    }
    ...
    if (jsonOutput) {
      ...
      print(
          'Generated Release Verification Pipeline Plan Preview for "${result.packageName}":');
      print('══════════════════════════════════════════════════════════════');
      print('Package Version : ${plan.version}');
      print('Policy Profile  : ${plan.policy.profile}');
      print('Total Stages    : ${plan.stages.length}');
      print(
          'Release Gate    : ${result.isReady ? "READY FOR RELEASE ✓" : "NOT READY / BLOCKED ✗"}');
      print('══════════════════════════════════════════════════════════════');
    }
...
// ─────────────────────────────────────────────────────────────────────────────
// template release-notes <template-id>
// ─────────────────────────────────────────────────────────────────────────────

/// Subcommand: `fps template release-notes <template-id>`
///
/// Previews or generates a release notes & documentation bundle for a template.
class TemplateReleaseNotesCommand extends FpsCommand {
  @override
  final String name = 'release-notes';

  @override
  final String description =
      'Preview or generate release notes & documentation bundle for a template.';

  TemplateReleaseNotesCommand() {
    argParser.addOption(
      'version',
      abbr: 'v',
      help: 'Version constraint/target for template release notes.',
      defaultsTo: '1.0.0',
    );
    argParser.addOption(
      'profile',
      abbr: 'p',
      help: 'Documentation bundle profile (standard, strict, custom).',
      defaultsTo: 'standard',
    );
    argParser.addOption(
      'output',
      abbr: 'o',
      help:
          'Target output directory when writing documentation bundle to disk.',
      defaultsTo: 'doc/release',
    );
    argParser.addFlag(
      'write',
      negatable: false,
      help: 'Write generated release documentation bundle directly to disk.',
    );
    argParser.addFlag(
      'json',
      negatable: false,
      help: 'Output release documentation bundle result as JSON.',
    );
  }
