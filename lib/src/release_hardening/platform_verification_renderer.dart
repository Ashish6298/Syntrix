...
import 'dart:convert';
import 'package:syntrix/src/release_hardening/platform_verification_models.dart';
...
class PlatformVerificationRenderer {
  ...
  static String renderJson(PlatformVerificationReport report,
      {bool pretty = true}) {
    final encoder =
  ...
  static String renderAsciiPlatformDashboard(
      PlatformVerificationReport report) {
    final buffer = StringBuffer();

    buffer.writeln(
        '┌────────────────────────────────────────────────────────────┐');
    buffer.writeln(
        '│ PHASE 11.4 — TARGET PLATFORM VERIFICATION MATRIX           │');
    buffer.writeln(
        '├────────────────────────────────────────────────────────────┤');
    buffer.writeln(
    ...
    buffer.writeln(
        '├────────────────────────────────────────────────────────────┤');
    buffer.writeln(
        '│ Platforms Audited: ${report.totalPlatformsAudited.toString().padRight(2)} | Total Checks Run: ${report.totalChecksRun.toString().padRight(3)} | Status: ${report.isAllPlatformsVerified ? "VERIFIED" : "FAILED"}   │');
    buffer.writeln(
        '│ Multiplatform Compliance: ${(report.isAllPlatformsVerified ? "100% PASS (ALL PLATFORMS)" : "FAILURES DETECTED").padRight(32)} │');
    buffer.writeln(
        '└────────────────────────────────────────────────────────────┘');
  ...
  static String renderMarkdown(PlatformVerificationReport report) {
    ...
    buffer.writeln();
    buffer.writeln(
        '**Platform Verification Status:** `${report.isAllPlatformsVerified ? "VERIFIED (100% Cross-Platform)" : "FAILED"}`  ');
    buffer.writeln('**Target Release Version:** `v${report.targetVersion}`  ');
    buffer.writeln(
        '**Platforms Audited:** `${report.totalPlatformsAudited}` (Android, iOS, Web, Windows, macOS, Linux)  ');
    buffer
        .writeln('**Total Platform Checks Run:** `${report.totalChecksRun}`  ');
    ...
    for (final item in report.checkItems) {
      buffer.writeln(
          '| **${item.platform.label}** | ${item.dimension.label} | ${item.status.symbol} `${item.status.label}` | ${item.verificationDetails} |');
    }
    buffer.writeln();
