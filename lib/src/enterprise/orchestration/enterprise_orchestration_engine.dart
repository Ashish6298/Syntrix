...
import 'package:syntrix/src/enterprise/rbac/rbac_engine.dart';
import 'package:syntrix/src/enterprise/credentials/enterprise_credential_manager.dart';
import 'package:syntrix/src/enterprise/workers/enterprise_worker_manager.dart';
...
class EnterpriseWorkflowEngine {
  ...
  void registerStageHandler(
      WorkflowStageType type, StageExecutionHandler handler) {
    ...
    for (final stage in plan.stages) {
      for (final dep in stage.dependsOnStageIds) {
        if (!stageIds.contains(dep)) {
          errors.add(
              'Stage "${stage.stageId}" depends on non-existent stage "$dep"');
        }
      }
    }
    ...
    for (final s in plan.stages) {
      if (!visited.contains(s.stageId)) {
        if (hasCycle(s.stageId)) {
          errors.add(
              'Circular dependency detected involving stage "${s.stageId}"');
          break;
        }
      }
    }
  ...
  Future<EnterpriseWorkflowExecutionResult> executeWorkflow(
    ...
    EnterpriseWorkflowExecutionStatus overallStatus =
        EnterpriseWorkflowExecutionStatus.completed;
    ...
    for (final stage in plan.stages) {
      ...
      if (isAborted) {
        completedResults[stage.stageId] = OrchestrationStageResult(
          ...
          errorMessage:
              'Skipped due to upstream failure or gate blockage ($blockedStageId)',
          durationMs: 0,
          startedAt: DateTime.now(),
          completedAt: DateTime.now(),
        );
      ...
      final stageRes =
          await _executeStageWithRetries(stage, plan, completedResults);
      ...
      if (!stageRes.isPassed) {
        if (stage.isMandatoryGate) {
          ...
          blockedStageId = stage.stageId;
          overallStatus =
              stageRes.status == EnterpriseWorkflowStageStatus.cancelled
    ...
    final passedCount = allResults
        .where((s) => s.status == EnterpriseWorkflowStageStatus.passed)
        .length;
    final failedCount = allResults
        .where((s) => s.status == EnterpriseWorkflowStageStatus.failed)
        .length;
    final skippedCount = allResults
        .where((s) =>
            s.status == EnterpriseWorkflowStageStatus.skipped ||
    ...
    while (attempts < maxAttempts) {
      ...
      try {
        ...
        if (_customHandlers.containsKey(stage.stageType)) {
          res = await _customHandlers[stage.stageType]!(
                  stage, plan, previousResults)
        ...
        lastResult = OrchestrationStageResult(
          ...
          errorMessage:
              'Stage execution timed out after ${stage.timeout.inSeconds}s',
          retryCount: attempts - 1,
    ...
    switch (stage.stageType) {
      ...
      case WorkflowStageType.securityAudit:
        if (securityEngine != null) {
          final eval = await securityEngine!.assessCompliance(
            operationalEvidence: {
              'detected_secrets_count':
                  stage.stageConfig['mock_secret_detected'] == true ? 2 : 0,
            },
          );
          return OrchestrationStageResult(
            stageId: stage.stageId,
            stageType: stage.stageType,
            name: stage.name,
            status: eval.isCompliant
                ? EnterpriseWorkflowStageStatus.passed
                : EnterpriseWorkflowStageStatus.failed,
            outputData: {
              'security_passed': eval.isCompliant,
              'failed_gates': eval.failedGates,
            },
            errorMessage: eval.isCompliant
                ? null
                : 'Security audit failed: ${eval.failedGates.join(", ")}',
            durationMs: 30,
            startedAt: start,
            completedAt: DateTime.now(),
          );
      ...
      case WorkflowStageType.dependencyGovernance:
        if (dependencyEngine != null) {
          ...
          return OrchestrationStageResult(
            ...
            errorMessage: scan.isCompliant
                ? null
                : 'Dependency governance violations detected.',
            durationMs: 20,
            startedAt: start,
            completedAt: DateTime.now(),
          );
      ...
      case WorkflowStageType.artifactBuild:
        return OrchestrationStageResult(
          ...
          outputData: {
            'archive_name':
                '${plan.targetPackageName}-${plan.targetVersion}.tar.gz',
      ...
      case WorkflowStageType.humanApproval:
        ...
        if (!mockApproved) {
          return OrchestrationStageResult(
            ...
            status: EnterpriseWorkflowStageStatus.failed,
            errorMessage:
                'Human approval rejected by required release governance reviewer.',
            durationMs: 15,
            startedAt: start,
            completedAt: DateTime.now(),
          );
      ...
      case WorkflowStageType.packagePublish:
        return OrchestrationStageResult(
          ...
          outputData: {
            'repository_url':
                'https://pub.enterprise.internal/packages/${plan.targetPackageName}',
            'status': 'PUBLISHED',
          },
          durationMs: 50,
          startedAt: start,
