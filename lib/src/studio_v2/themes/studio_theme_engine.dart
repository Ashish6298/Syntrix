...
class StudioThemeEngine {
  ...
  void _registerDefaultPackageThemes() {
    registerTheme(const StudioThemeDescriptor(
      ...
      name: 'Deep Space',
      description:
          'Abyssal cosmic midnight dark palette with indigo and violet nebula particles.',
      palette: [
    ...
    registerTheme(const StudioThemeDescriptor(
      ...
      name: 'Milky Way',
      description:
          'Stellar spiral galaxy blend with opalescent starlight whites and soft cyan trails.',
      palette: [
    ...
    registerTheme(const StudioThemeDescriptor(
      ...
      name: 'Nebula Storm',
      description:
          'Vibrant interstellar storm with electric magenta, purple dust, and cyan ionization.',
      palette: [
    ...
    registerTheme(const StudioThemeDescriptor(
      ...
      name: 'Quantum Void',
      description:
          'Deep pitch black void accented by high-contrast fluorescent neon emerald highlights.',
      palette: [
    ...
    registerTheme(const StudioThemeDescriptor(
      ...
      name: 'Solar Flare',
      description:
          'Blazing coronal plasma energy with fiery amber, crimson corona, and gold particles.',
      palette: [
    ...
    registerTheme(const StudioThemeDescriptor(
      ...
      name: 'Cyber Galaxy',
      description:
          'Retro futuristic synthwave grid with neon cyan, hot pink, and dark obsidian backdrop.',
      palette: [
    ...
    registerTheme(const StudioThemeDescriptor(
      ...
      name: 'Aurora Cosmos',
      description:
          'Polar geomagnetic aurora shimmering across cosmic sky with teal and lavender ribbons.',
      palette: [
  ...
  void applyTheme(String themeId) {
    ...
    if (theme != null) {
      controller.updateConfiguration(
          (cfg) => cfg.copyWith(selectedThemeId: theme.id));
      _logger.info('Applied theme "${theme.name}" to workspace.');
    }
  }

  /// Duplicate an existing theme to create a custom variation.
  StudioThemeDescriptor duplicateTheme(String sourceThemeId,
      {required String newId, required String newName}) {
    final source = _themeCatalog[sourceThemeId];
    if (source == null) {
      throw ArgumentError(
          'Source theme "$sourceThemeId" not found in catalog.');
    }
  ...
  String generateThemeCode(String themeId) {
    ...
    buffer.writeln(
        '// Production-ready UniverseTheme configuration generated by Syntrix Studio v2');
    buffer.writeln('import \'package:flutter/material.dart\';');
    buffer.writeln();
    buffer.writeln('class ${theme.name.replaceAll(" ", "")}Theme {');
    buffer.writeln('  static const String themeId = \'${theme.id}\';');
    buffer.writeln(
        '  static const Color backgroundColor = Color(${theme.backgroundColor.replaceAll("#", "0xFF")});');
    buffer.writeln(
        '  static const Color particleGlowColor = Color(${theme.particleGlowColor.replaceAll("#", "0xFF")});');
    buffer.writeln();
    buffer.writeln('  static const List<Color> palette = [');
    for (final c in theme.palette) {
      buffer
          .writeln('    Color(${c.hex.replaceAll("#", "0xFF")}), // ${c.role}');
    }
    buffer.writeln('  ];');
    buffer.writeln('}');
