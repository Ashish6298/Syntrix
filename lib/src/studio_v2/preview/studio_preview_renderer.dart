...
import 'dart:convert';
import 'package:syntrix/src/studio_v2/preview/studio_preview_models.dart';
...
class StudioPreviewRenderer {
  ...
  static String renderJson(LivePreviewSessionState state,
      {bool pretty = true}) {
    final encoder =
  ...
  static String renderAsciiViewport(LivePreviewSessionState state) {
    ...
    buffer.writeln(
        '┌──────────────────────────────────────────────────────────────┐');
    buffer.writeln(
        '│ LIVE PREVIEW VIEWPORT: ${state.activeLoaderId.padRight(20)} [${state.viewportMode.id.toUpperCase()}] │');
    buffer.writeln(
        '├──────────────────────────────────────────────────────────────┤');
    buffer.writeln(
        '│ [Status: ${state.playbackState.id.toUpperCase().padRight(10)}] │ [Speed: ${cfg.animationSpeed.toStringAsFixed(1)}x] │ [Scale: ${cfg.scale.toStringAsFixed(1)}x] │ [Particles: ${cfg.particleCount.toString().padLeft(4)}] │');
    buffer.writeln(
        '├──────────────────────────────────────────────────────────────┤');
    buffer.writeln(
        '│                                                              │');
    ...
    buffer.writeln(
        '├──────────────────────────────────────────────────────────────┤');
    buffer.writeln(
        '│ Telemetry: FPS: ${m.currentFps.toStringAsFixed(1).padLeft(5)} │ Frame: ${m.currentFrameTimeMs.toStringAsFixed(1).padLeft(5)}ms │ Avg: ${m.averageFps.toStringAsFixed(1).padLeft(5)} FPS │ Drops: ${m.droppedFrames} │');
    buffer.writeln(
  ...
  static String renderMarkdown(LivePreviewSessionState state) {
    ...
    buffer.writeln(
        '**Playback State:** `${state.playbackState.id.toUpperCase()}`  ');
    buffer.writeln(
        '**Viewport Mode:** `${state.viewportMode.id.toUpperCase()}`  ');
    ...
    buffer.writeln('|---|---|');
    buffer.writeln(
        '| **Current Framerate** | `${m.currentFps.toStringAsFixed(1)} FPS` |');
    buffer.writeln(
        '| **Average Framerate** | `${m.averageFps.toStringAsFixed(1)} FPS` |');
    buffer.writeln(
        '| **Frame Time Duration** | `${m.currentFrameTimeMs.toStringAsFixed(2)} ms` |');
    buffer.writeln(
        '| **Min / Max Frame Time** | `${m.minFrameTimeMs.toStringAsFixed(2)} ms / ${m.maxFrameTimeMs.toStringAsFixed(2)} ms` |');
    buffer
        .writeln('| **Total Frames Rendered** | `${m.totalFramesRendered}` |');
    buffer.writeln('| **Dropped Frames (<50 FPS)** | `${m.droppedFrames}` |');
    buffer.writeln(
        '| **Active Particle Simulations** | `${m.currentParticleCount}` |');
    buffer.writeln();
    ...
    buffer.writeln(
        '- **Shaders:** `${cfg.shadersEnabled ? "Enabled" : "Disabled"}`');
    buffer.writeln(
        '- **Interactive Gestures:** `${cfg.isInteractive ? "Enabled" : "Disabled"}`');
    buffer.writeln();
