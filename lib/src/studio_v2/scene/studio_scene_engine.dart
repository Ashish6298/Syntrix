...
class StudioSceneBuilderEngine {
  ...
  static VisualSceneDescriptor _createDefaultScene() {
    ...
    return VisualSceneDescriptor(
      ...
      description:
          'Default composite scene containing Background, Galaxy Orbit, Cosmic Dust, and Gesture Overlay.',
      layers: const [
        SceneComponentLayer(
          layerId: 'layer_bg_01',
  ...
  void addLayer(SceneComponentLayer layer) {
    final updatedLayers = List<SceneComponentLayer>.from(_activeScene.layers)
      ..add(layer);
    ...
    _logger
        .info('Added layer "${layer.name}" (ID: ${layer.layerId}) to scene.');
    _notify();
  }
  ...
  bool removeLayer(String layerId) {
    final updatedLayers =
        _activeScene.layers.where((l) => l.layerId != layerId).toList();
  ...
  void configureLayer(String layerId,
      {bool? isVisible, double? opacity, Map<String, dynamic>? config}) {
    final updatedLayers = _activeScene.layers.map((l) {
      if (l.layerId == layerId) {
        return l.copyWith(
  ...
  void saveScene() {
    _savedScenes[_activeScene.sceneId] = _activeScene;
    _logger
        .info('Saved scene: ${_activeScene.sceneId} ("${_activeScene.name}")');
  }
  ...
  String generateSceneFlutterCode() {
    ...
    buffer.writeln(
        '// Production-ready Flutter Stack Scene generated by Syntrix Scene Builder');
    ...
    for (final layer in _activeScene.layers.where((l) => l.isVisible)) {
      buffer.writeln(
          '        // Layer: ${layer.name} (${layer.layerType.displayName}, z-index: ${layer.zIndex})');
      buffer.writeln('        Opacity(');
      buffer.writeln('          opacity: ${layer.opacity},');
      buffer.writeln('          child: SceneComponent(');
      buffer.writeln(
          '            componentRef: \'${layer.componentReference}\',');
